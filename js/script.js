        ...
        // ================= 配置区 (严禁改动功能逻辑) =================
        const DIFY_API_KEY = "app-SpCWzrDgknWbtrx7kD0XTeyB"; 
        const DIFY_API_URL = "https://6913524e.r31.cpolar.top/v1"; 
        const USERS = { "abee": "123456", "guest": "888888" };
        const OPENING_STATEMENT = "你好！我是你的倾听艺术助手。很高兴能陪你一起开启这段关于深度沟通的反思之旅。准备好了吗？我们可以从第一个问题开始。\n\n1. 你心目中最优秀的倾听人是谁？他/她都有哪些良好的倾听表现？(比如，他/她会打断你的谈话吗？会提出让你感兴趣的问题吗？会认可你讲的话吗？)你与这个人在一起是怎样的感觉？从这个人的身上，你能学到什么可以让你成为更好的倾听人的东西吗？";
        // ===============================================================
        ...
        function handleLogout() {
            ...
            setTimeout(() => {
                ...
                chatSec.style.opacity = '1'; chatSec.style.transform = 'none'; chatSec.style.filter = 'none';
                const loginSec = document.getElementById('login-section');
                loginSec.classList.remove('hidden');
                loginSec.style.opacity = '1'; loginSec.style.transform = 'none'; loginSec.style.filter = 'none';
        ...
        async function fetchHistory() {
            try {
                const response = await fetch(`${DIFY_API_URL}/conversations?user=${currentUser}`, {
        ...
        async function loadConversation(id) {
            conversationId = id;
            document.getElementById('display-cid').innerText = `ID: ${id.substring(0, 5)}..`;
            const chatBox = document.getElementById('chat-box');
            chatBox.innerHTML = '<div class="text-slate-300 text-[10px] font-black text-center py-32 animate-pulse tracking-[1em] uppercase italic">正在解码加密数据...</div>';
            try {
                const response = await fetch(`${DIFY_API_URL}/messages?user=${currentUser}&conversation_id=${id}`, {
                    headers: { 'Authorization': `Bearer ${DIFY_API_KEY}` }
                });
                const data = await response.json();
                chatBox.innerHTML = "";
                ...
                fetchHistory();
            } catch (err) { appendMessage('ai', "❌ 链路故障，无法恢复档案。"); }
        }
                    ...
                    for (const line of lines) {
                        ...
                        try {
                            ...
                            if (data.conversation_id && !conversationId) {
                                ...
                                document.getElementById('display-cid').innerText = `ID: ${conversationId.substring(0, 5)}..`;
                                fetchHistory(); 
                            }
        ...
        // 精雕气泡生成逻辑：引入 max-w-2xl w-full mx-auto
        function createAiMessagePlaceholder() {
            ...
            inner.className = "max-w-2xl w-full mx-auto flex gap-5 md:gap-10 items-start";
            ...
            // 对话气泡宽度进一步收窄，确保与输入框对齐感
            content.className = "bg-white text-slate-800 p-6 md:p-10 rounded-[3rem] rounded-tl-xl border border-slate-50 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.05)] max-w-[75%] md:max-w-[65%] text-base md:text-[17px] whitespace-pre-wrap leading-[1.8] font-medium tracking-tight";
            content.innerText = "...";
        ...
        function appendMessage(role, text) {
            ...
            const inner = document.createElement('div');
            inner.className = `max-w-2xl w-full mx-auto flex gap-5 md:gap-10 items-start ${role === 'user' ? 'flex-row-reverse' : ''}`;
            ...
            const content = document.createElement('div');
            if (role === 'user') {
                content.className = "bg-slate-900 text-white p-6 md:p-10 rounded-[3rem] rounded-tr-xl shadow-[0_30px_60px_-15px_rgba(26,32,44,0.35)] max-w-[75%] md:max-w-[65%] text-base md:text-[17px] whitespace-pre-wrap leading-[1.8] font-medium tracking-tight";
            } else {
                content.className = "bg-white text-slate-800 p-6 md:p-10 rounded-[3rem] rounded-tl-xl border border-slate-50 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.05)] max-w-[75%] md:max-w-[65%] text-base md:text-[17px] whitespace-pre-wrap leading-[1.8] font-medium tracking-tight";
            }
